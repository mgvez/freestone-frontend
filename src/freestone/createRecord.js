	...
	const newRecordPromise = table.fields.reduce((recordPromise, field) => {
		...
		return recordPromise.then(record => {
			if (field.type === TYPE_ORDER) {
				...
				case 'now()': {
					const d = new Date();
					val = d.getDate().toString().padStart(2, '0') + '-' + (d.getMonth() + 1).toString().padStart(2, '0') + '-' + d.getFullYear() + ' ' + d.getHours().toString().padStart(2, '0') + ':' + d.getMinutes().toString().padStart(2, '0') + ':' + d.getSeconds().toString().padStart(2, '0');
					break;
				}
				//ajax values
				case 'increment': {
